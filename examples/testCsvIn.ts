import { Workbook, ValueType } from "../src/index.js";
...
const assertDate = function (address, cell, expected) {
  assert(cell.type === ValueType.Date, `expected ${address} type to be Date, was ${cell.type}`, "");
  assert(
...
wb.csv.readFile(filename, options).then(() => {
  ...
  assert(
    ws.getCell("A2").type === ValueType.Number,
  ...
  assert(
    ws.getCell("B2").type === ValueType.String,
  ...
  assert(
    ws.getCell("C2").type === ValueType.Number,
  ...
  assert(
    ws.getCell("C5").type === ValueType.Number,
