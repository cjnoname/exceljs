...
const style_xform_1 = __importDefault(require("./style-xform"));
const dxf_xform_1 = __importDefault(require("./dxf-xform"));
// custom numfmt ids start here
const NUMFMT_BASE = 164;
// =============================================================================
// StylesXform is used to generate and parse the styles.xml file
// it manages the collections of fonts, number formats, alignments, etc
class StylesXform extends base_xform_1.default {
    constructor(initialise) {
        super();
        this.map = {
            numFmts: new list_xform_1.default({ tag: 'numFmts', count: true, childXform: new numfmt_xform_1.default() }),
            fonts: new list_xform_1.default({
                tag: 'fonts',
                count: true,
                childXform: new font_xform_1.default(),
                $: { 'x14ac:knownFonts': 1 },
            }),
            fills: new list_xform_1.default({ tag: 'fills', count: true, childXform: new fill_xform_1.default() }),
            borders: new list_xform_1.default({ tag: 'borders', count: true, childXform: new border_xform_1.default() }),
            cellStyleXfs: new list_xform_1.default({ tag: 'cellStyleXfs', count: true, childXform: new style_xform_1.default() }),
            cellXfs: new list_xform_1.default({
                tag: 'cellXfs',
                count: true,
                childXform: new style_xform_1.default({ xfId: true }),
            }),
            dxfs: new list_xform_1.default({ tag: 'dxfs', always: true, count: true, childXform: new dxf_xform_1.default() }),
            // for style manager
            ...
            style: new style_xform_1.default({ xfId: true }),
    ...
    addStyleModel(model, cellType) {
        ...
        const style = {};
        const type = cellType || enums_1.default.ValueType.Number;
        if (model.numFmt) {
    ...
    getStyleModel(id) {
        ...
        // number format
        if (style.numFmtId) {
            const numFmt = this.index.numFmt[style.numFmtId] || numfmt_xform_1.default.getDefaultFmtCode(style.numFmtId);
            if (numFmt) {
                model.numFmt = numFmt;
            }
        }
    ...
    _addNumFmtStr(formatCode) {
        ...
        let index = numfmt_xform_1.default.getDefaultFmtId(formatCode);
        if (index !== undefined)
            return index;
        // check if already in
...
StylesXform.STATIC_XFORMS = {
    cellStyles: new static_xform_1.default({
        tag: 'cellStyles',
        $: { count: 1 },
        c: [{ tag: 'cellStyle', $: { name: 'Normal', xfId: 0, builtinId: 0 } }],
    }),
    dxfs: new static_xform_1.default({ tag: 'dxfs', $: { count: 0 } }),
    tableStyles: new static_xform_1.default({
        tag: 'tableStyles',
        $: { count: 0, defaultTableStyle: 'TableStyleMedium2', defaultPivotStyle: 'PivotStyleLight16' },
    }),
    extLst: new static_xform_1.default({
        tag: 'extLst',
        c: [
            {
                tag: 'ext',
...
class StylesXformMock extends StylesXform {
    ...
    get dateStyleId() {
        if (!this._dateStyleId) {
            const dateStyle = {
                numFmtId: numfmt_xform_1.default.getDefaultFmtId('mm-dd-yy'),
            };
