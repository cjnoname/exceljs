...
class Range {
    ...
    setTLBR(t, l, b, r, s) {
        if (arguments.length < 4) {
            // setTLBR(tl, br, s)
            const tl = col_cache_1.default.decodeAddress(t);
            const br = col_cache_1.default.decodeAddress(l);
            this.model = {
        ...
        switch (argv.length) {
            ...
            case 1: {
                ...
                if (value instanceof Range) {
                    ...
                    // [sheetName!]tl:br
                    const tlbr = col_cache_1.default.decodeEx(value);
                    if (tlbr.top) {
                        this.model = {
                            top: tlbr.top,
                            left: tlbr.left,
    ...
    expandToAddress(addressStr) {
        const address = col_cache_1.default.decodeEx(addressStr);
        this.expand(address.row, address.col, address.row, address.col);
    }
    get tl() {
        return col_cache_1.default.n2l(this.left) + this.top;
    }
    get $t$l() {
        return `$${col_cache_1.default.n2l(this.left)}$${this.top}`;
    }
    get br() {
        return col_cache_1.default.n2l(this.right) + this.bottom;
    }
    get $b$r() {
        return `$${col_cache_1.default.n2l(this.right)}$${this.bottom}`;
    }
    ...
    forEachAddress(cb) {
        for (let col = this.left; col <= this.right; col++) {
            for (let row = this.top; row <= this.bottom; row++) {
                cb(col_cache_1.default.encodeAddress(row, col), row, col);
            }
        }
    }
}
