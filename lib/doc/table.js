...
class Table {
    ...
    validate() {
        ...
        table.tl = col_cache_1.default.decodeAddress(table.ref);
        const { row, col } = table.tl;
        assert(row > 0, 'Table must be on valid row');
        assert(col > 0, 'Table must be on valid col');
        const { width, filterHeight, tableHeight } = this;
        // autoFilterRef is a range that includes optional headers only
        table.autoFilterRef = col_cache_1.default.encode(row, col, row + filterHeight - 1, col + width - 1);
        // tableRef is a range that includes optional headers and totals
        table.tableRef = col_cache_1.default.encode(row, col, row + tableHeight - 1, col + width - 1);
    ...
    commit() {
        ...
        // check things are ok first
        this.validate();
        const ref = col_cache_1.default.decodeAddress(this._cache.ref);
